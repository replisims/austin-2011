...
|Scenario| Correlation between covariates| Covariate type| 
|Independent normal|
|Correlated normal|
|Mixed 1|
|Mixed 2| 
...
\subsubsection{Within - matched pair diffence}
Austin writes "Within the propensity-score matched sample, let $Y_{T,i}$ and $Y_{C,i} denote the outcome for the treated and untreated subjects in the ith matched set, respectively. Then let $d_i = Y_{C,i} - Y_{T,i} denoted the within-matched pair difference in outcome between  treated and untreated subject."
We found this definition to by highly unconventional.
Furthermore, this definition led to some inconsistencies.
It would follow that the true treatment effect is not identical to $beta$ but rather has an opposite sign which makes code implementation slightly confusing.
In addition, in all figures Austin reports the "True delta" as as positive thus identical to $beta$.
For reasons of convention as well as consistency and ease of implementation we thus assumed that the above definition contained a typo.
We hence assumed the effect i.e. difference in mean to refer to subtracting the outcome of the the untreated from that of the treated. 
...
\subsubsection{Reduction in bias - Figure 1}

\subsubsection{Mean squared error - Figure 2}

\subsubsection{Coverage of 95% CIs - Figure 3}

\subsubsection{Empirical type I error rate - Figure 4}

\subsubsection{Reduction in bias - Figure 5}

\subsubsection{Mean squared error - Figure 6}

\subsubsection{Coverage of 95% CIs - Figure 7}
...
Replication was rather straight forward. 
We attribute this to the rather simple simulation design as well the replicators' experience with the ...
However, there were some details which were undefined or counter to our experience 
The values of $\alpha_{0,treat}$ and $\alpha_{0,outcome}$ were not only not provided for any scenario but the "independent normal scenario" their adaptation was also discussed in a rather odd place i.e. the end of the section 3.1.2. detailing the data generation of the continuous outcomes even though these parameters pertained to the data generation for binary outcomes discussed in section 3.1.1.
